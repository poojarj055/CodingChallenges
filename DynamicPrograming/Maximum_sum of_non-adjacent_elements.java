...
public class Solution {
	public static int maximumNonAdjacentSum(ArrayList<Integer> nums) {
		...
		Arrays.fill(cache,-1);
		return maximumNonAdjacentSumRecur(nums,n-1,cache);

	}
	 static int maximumNonAdjacentSumRecur(ArrayList<Integer> nums, int i, int []cache){
		if(i<0){
			return 0;
		}
		if(i==0){
		...
		int choose=maximumNonAdjacentSumRecur(nums,i-2,cache)+nums.get(i);
		int skip=maximumNonAdjacentSumRecur(nums,i-1,cache);
