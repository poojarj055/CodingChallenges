...
class Solution {
    public int uniquePaths(int m, int n) {
        ...
        return uniquePathsRecur(m, n, row, col,cache);
    }
    int uniquePathsRecur(int m, int n, int row, int col, int[][]cache){
        if(row==m-1 && col==n-1){
            return 1;
        }
        if(row>m-1 || col>n-1){
        ...
        int bottom=uniquePathsRecur(m,n,row+1,col,cache);
        int right=uniquePathsRecur(m,n,row,col+1,cache);
